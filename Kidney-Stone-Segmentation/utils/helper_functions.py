...
def plot_conf_mat(rootdir, labels_):
    filelist = os.listdir(rootdir)
    filenum = len(filelist)
    for i in range(0,filenum):
        current_file_fulldir = rootdir + filelist[i]
        current_file_data = h5py.File(current_file_fulldir, 'r')
        GT_label_current_batch = np.array(current_file_data['X'], dtype=np.uint16).ravel()
        Pred_label_current_batch = np.array(current_file_data['Y'], dtype=np.uint16).ravel()
        confusion_matrix_raw_current_batch = confusion_matrix(GT_label_current_batch, Pred_label_current_batch, labels=labels_, normalize=None)
        if i == 0:
            confusion_matrix_raw = confusion_matrix_raw_current_batch
        else:
            confusion_matrix_raw = confusion_matrix_raw + confusion_matrix_raw_current_batch
    confusion_matrix_norm = confusion_matrix_raw.astype('float') / confusion_matrix_raw.sum(axis=1)[:, np.newaxis]
    ...
    fig = plt.figure(figsize=(len(labels_)*3, len(labels_)*2))
    sns.heatmap(confusion_matrix_norm, cmap='Blues', annot=annots, fmt='', annot_kws={'fontsize': 14}, xticklabels=labels_, yticklabels=labels_, vmax=1)
    plt.title('Confusion Matrix', fontsize=24)
    plt.xlabel("Predicted", fontsize=14)
    plt.ylabel("True", fontsize=14)
    return fig, confusion_matrix_raw
...
def prepareTrainDict_V3(image_batch, model_depth, model_type):
    def approximate(inp, w_len, length, width, num_channel, batch_size):
        ops = np.empty((batch_size, length//w_len, width//w_len, num_channel), dtype=np.int16)
        for i in range(0, batch_size):
            for c in range(0, num_channel):
                img_temp = Image.fromarray(inp[i,:,:,c], mode='L')
                img_temp = np.asarray(img_temp.resize((length//w_len, width//w_len), method='nearest'))
                ops[i,:,:,c] = img_temp
        return ops
...
def prepareTrainDict_V4(image_batch, model_depth, model_type, normalizing_factor_img, normalizing_factor_msk):
    def approximate(inp, w_len, length, width):
        img_temp = np.asarray(tf.image.resize(inp*(normalizing_factor_img/normalizing_factor_msk), (length//w_len, width//w_len), method='nearest')/(normalizing_factor_img/normalizing_factor_msk), dtype=np.int16)
        return img_temp
    
    # Prcoess Image
    image_batch_arr = np.asarray(image_batch, dtype=np.int16)
    if len(image_batch_arr.shape) == 3:
        image_batch_arr = np.expand_dims(image_batch_arr, axis=3)
    Y_Train_dict = {}
    Y_Train_dict['out'] = image_batch_arr
    for i in range(1, (model_depth+1)):
        name = f'level{i}'
        if model_type == 'UNet':
            Y_Train_dict[name] = approximate(image_batch, 2**i, image_batch.shape[1], image_batch.shape[2])
        elif model_type == 'UNetPP':
            Y_Train_dict[name] = image_batch_arr

    return Y_Train_dict


def prepareTrainDict_V5(image_batch, model_depth, model_type, normalizing_factor_img, normalizing_factor_msk):
    def approximate(inp, w_len):
        ops = np.asarray((tf.keras.layers.MaxPooling2D(pool_size=(w_len, w_len))(inp*(normalizing_factor_img/normalizing_factor_msk)))/(normalizing_factor_img/normalizing_factor_msk), dtype=np.int16)
        return ops
    
    # Prcoess Image
    image_batch = np.asarray(image_batch, dtype=np.int16)
    if len(image_batch.shape) == 3:
        image_batch = np.expand_dims(image_batch, axis=3)
    out = {}
    Y_Train_dict = {}
...
def assd(y_true, y_pred, spacing=None):
    ...
    s_true = y_true ^ binary_erosion(y_true, structure=footprint)
    s_pred = y_pred ^ binary_erosion(y_pred, structure=footprint)
