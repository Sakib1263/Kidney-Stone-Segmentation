...
class CustomDataGenerator(tf.keras.utils.Sequence):
    ...
    def __data_generation(self, list_IDs_temp):
        ...
        if self.patchify == True:
            img_patch_counter = 0
            msk_patch_counter = 0
            x = np.empty((10000, self.patch_shape[0], self.patch_shape[1], self.num_img_channel), dtype=np.float64)
            y = np.empty((10000, self.patch_shape[0], self.patch_shape[1], self.num_msk_channel), dtype=np.int16)
        else:
            x = np.empty((self.batch_size, self.img_size[0], self.img_size[1], self.num_img_channel), dtype=np.float64)
            y = np.empty((self.batch_size, self.img_size[0], self.img_size[1], self.num_msk_channel), dtype=np.int16)
        # Generate data
        for i, ID in enumerate(list_IDs_temp):
            ...
            if self.patchify == True:
                ...
                x[img_patch_counter:img_patch_counter + num_patches,:,:,:] = np.reshape(np.squeeze(patches), 
                                                                                        (num_patches, self.patch_shape[0], self.patch_shape[1], self.num_img_channel))
            ...
            msk = tf.keras.utils.load_img(path=f'{self.msk_dir}/{ID}',
                                            ...
                                            interpolation='nearest',
                                            keep_aspect_ratio=False
                                            ) / self.norm_factor_msk
            if self.patchify == True:
                patches, num_patches = create_patches(msk, self.patch_shape, self.overlap_ratio)
                y[msk_patch_counter:msk_patch_counter + num_patches,:,:,0] = np.reshape(np.squeeze(patches), 
                                                                                        (num_patches, self.patch_shape[0], self.patch_shape[1], self.num_msk_channel))
        ...
        if self.deep_supervision == True:
            y = prepareTrainDict_V4(y, self.model_depth, self.ds_type, self.norm_factor_img, self.norm_factor_msk)  # V4 is the best one so far
        return x, y
